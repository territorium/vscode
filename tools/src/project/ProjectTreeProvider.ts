...
export class ProjectTreeProvider implements vscode.TreeDataProvider<ProjectNode> {
    ...
    public async getTreeItem(element: ProjectNode): Promise<ProjectTreeItem> {
        ...
        switch (element.getType()) {
            case "project":
                item.iconPath = {
                    dark: this._context.asAbsolutePath(path.join('icons', 'dark', 'symbol-method.svg')),
                    light: this._context.asAbsolutePath(path.join('icons', 'light', 'symbol-method.svg'))
                };
                break;

            case "model":
                item.iconPath = {
                    dark: this._context.asAbsolutePath(path.join('icons', 'dark', 'archive.svg')),
                    light: this._context.asAbsolutePath(path.join('icons', 'light', 'archive.svg'))
                };
                break;

            case "folder":
                item.iconPath = {
                    dark: this._context.asAbsolutePath(path.join('icons', 'dark', 'folder.svg')),
                    light: this._context.asAbsolutePath(path.join('icons', 'light', 'folder.svg'))
                };
                break;
            ...
            case "file":
                ...
                item.iconPath = {
                    dark: this._context.asAbsolutePath(path.join('icons', 'dark', 'file.svg')),
                    light: this._context.asAbsolutePath(path.join('icons', 'light', 'file.svg'))
                };
                break;
        }
